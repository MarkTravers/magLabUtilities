...
from magLabUtilities.signalutilities.hysteresis import HysteresisSignalBundle, XExpMremGedney071620
...
class CostEvaluator:
    ...
    def runCostFunction(self, gridNode:GridNode) -> GridNode:
        ...
        hCoop = gridNode.coordList[4]
        hAnh = gridNode.coordList[5]

        virginGen = XExpMremGedney071620(xInit=xInit, hCoercive=hCoercive, hCoop=hCoop, mNuc=mNuc, mSat=mSat, hAnh=hAnh, mRev=self.refBundle.signals['M'].independentThread.data[0], curveRegion='virgin')
        pRevGen = XExpMremGedney071620(xInit=xInit, hCoercive=hCoercive, hCoop=hCoop, mNuc=mNuc, mSat=mSat, hAnh=hAnh, mRev=self.refBundle.signals['M'].independentThread.data[self.pMAmpIndex]+1, curveRegion='reversal')
        nRevGen = XExpMremGedney071620(xInit=xInit, hCoercive=hCoercive, hCoop=hCoop, mNuc=mNuc, mSat=mSat, hAnh=hAnh, mRev=self.refBundle.signals['M'].independentThread.data[self.nMAmpIndex]-1, curveRegion='reversal')
        ...
        tWeightMatrix = np.vstack([self.refBundle.signals['M'].dependentThread.data, np.hstack([np.zeros(5), np.ones((195)), np.zeros((800))])])
